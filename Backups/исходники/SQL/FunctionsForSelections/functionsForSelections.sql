...
-- CORRELATED 1
-- все самолёты с количеством пассажиров
...
SELECT 
	price, tckt.flight, code
		FROM (SELECT * FROM tickets JOIN users ON tickets.passanger_id = users.user_id WHERE 
			price > (SELECT AVG(price) FROM tickets)) AS tckt
